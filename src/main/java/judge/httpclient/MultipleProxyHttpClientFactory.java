...
//    private final static Logger log = LoggerFactory.getLogger(MultipleProxyHttpClientFactory.class);
    ...
    public MultipleProxyHttpClientFactory(String jsonConfigPath) {
       // this.jsonConfig = new File(jsonConfigPath);
        String contextClassesPath=ApplicationContainer.serveletContext.getRealPath("/WEB-INF/classes") ;
    	File json=new File(jsonConfigPath);
    	if(!json.exists()){
    		json=new File(contextClassesPath+File.separator+jsonConfigPath);
    	}
